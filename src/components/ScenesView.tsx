...
export function ScenesView({ data, setData, onGenerateScript, status }: { data: ProjectData, setData: React.Dispatch<React.SetStateAction<ProjectData>>, onGenerateScript: () => void, status: string }) {
  ...
  return (
    <div className="max-w-6xl mx-auto py-12 px-8">
      ...
      {!data.stage07 && status !== 'generating_script' && (
        <div className="mt-8 flex justify-end">
          <button onClick={onGenerateScript} className="bg-[#f43f5e] text-black font-bold text-xs uppercase px-6 py-3 hover:bg-[#rose-600] transition-colors tracking-widest">
            Generate Script
          </button>
        </div>
      )}

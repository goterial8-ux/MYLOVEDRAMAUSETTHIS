...
export function FoundationView({ data, setData, onGenerateFoundation, onGenerateOutline, status }: { data: ProjectData, setData: React.Dispatch<React.SetStateAction<ProjectData>>, onGenerateFoundation: () => void, onGenerateOutline: () => void, status: string }) {
    ...
    <div className="max-w-6xl mx-auto py-12 px-8 space-y-8">
      <div className="border-b border-[#222] pb-4 mb-4">
        <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              {!data.stage01 && status !== 'generating_foundation' && (
                <button onClick={onGenerateFoundation} className="bg-[#f43f5e] text-black font-bold text-xs uppercase px-4 py-2 hover:bg-[#rose-600] transition-colors tracking-widest">
                  Generate Foundation DNA
                </button>
              )}
              {status === 'generating_foundation' && (
                <div className="text-[10px] text-[#f43f5e] font-mono uppercase tracking-widest animate-pulse">Generating DNA...</div>
              )}
            </div>
        </div>
      </div>
      ...
      <div className="mt-8 border-t border-[#222] pt-8">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {s00?.antiTemplateRiskReport?.map((risk: any, idx: number) => (
      ...
      {data.stage01 && (
        <div className="mt-8 border-t border-[#222] pt-8">
            ...
            {!data.stage02 && status !== 'generating_outline' && (
              <div className="mt-8 flex justify-end">
                <button onClick={onGenerateOutline} className="bg-[#f43f5e] text-black font-bold text-xs uppercase px-6 py-3 hover:bg-[#rose-600] transition-colors tracking-widest">
                  Generate Outline
                </button>
              </div>
            )}
            {status === 'generating_outline' && (
              <div className="mt-8 text-right text-[10px] text-[#f43f5e] font-mono uppercase tracking-widest animate-pulse">Generating Outline...</div>
            )}
        </div>
      )}

...
export function OutlineView({ data, setData, onGenerateScenes, status }: { data: ProjectData, setData: React.Dispatch<React.SetStateAction<ProjectData>>, onGenerateScenes: () => void, status: string }) {
  ...
  return (
    <div className="max-w-5xl mx-auto py-12 px-8">
      ...
      {!data.stage03 && status !== 'generating_scenes' && (
        <div className="mt-8 flex justify-end">
          <button onClick={onGenerateScenes} className="bg-[#f43f5e] text-black font-bold text-xs uppercase px-6 py-3 hover:bg-[#rose-600] transition-colors tracking-widest">
            Generate Scenes
          </button>
        </div>
      )}
      {status === 'generating_scenes' && (
        <div className="mt-8 text-right text-[10px] text-[#f43f5e] font-mono uppercase tracking-widest animate-pulse">Generating Scenes...</div>
      )}

...
export default function App() {
  ...
  const generateSetup = async (idea: string) => {
    setData(prev => ({ ...prev, rawIdea: idea, status: 'generating_setup', error: undefined }));
    try {
      const setupData = await fetchJson('/api/generate/setup', {
        method: 'POST',
  ...
  const generateFoundation = async () => {
    if (!data.stage00 || !data.rawIdea) return;
    setData(prev => ({ ...prev, status: 'generating_foundation', error: undefined }));
    try {
      const foundationData = await fetchJson('/api/generate/foundation', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ rawIdea: data.rawIdea, ideaSetupData: data.stage00 }),
      });
      setData(prev => ({ 
        ...prev, 
  ...
  const generateOutline = async () => {
    if (!data.stage00 || !data.stage01) return;
    setData(prev => ({ ...prev, status: 'generating_outline', error: undefined }));
    try {
      const outlineData = await fetchJson('/api/generate/outline', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ stage00: data.stage00, stage01: data.stage01 }), 
      });
      setData(prev => ({
        ...prev,
        stage02: outlineData,
  ...
  const generateScenes = async () => {
    if (!data.stage00 || !data.stage01 || !data.stage02) return;
    setData(prev => ({ ...prev, status: 'generating_scenes', error: undefined }));
    try {
      const projectSoFar = { foundation: { stage00: data.stage00, stage01: data.stage01 }, outline: data.stage02 };
      const scenesData = await fetchJson('/api/generate/scenes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const generateScript = async () => {
    if (!data.stage00 || !data.stage01 || !data.stage02 || !data.stage03) return;
    stopGenerationRef.current = false;
    setData(prev => ({ ...prev, status: 'generating_script', stage07: [], error: undefined }));
    ...
    try {
      ...
      for (let i = 1; i <= totalParts; i++) {
         ...
         const partOutline = data.stage02.complete9PartMacroOutline?.partSummaries?.find((p: any) => p.partNumber === String(i)) || data.stage02;
         const partScenes = data.stage03.completeSceneCardsByPart?.find((p: any) => p.partNumber === String(i)) || data.stage03;
         ...
         const scriptRes = await fetchJson('/api/generate/script-part', {
            ...
            body: JSON.stringify({
              partNumber: i,
              foundation: { stage00: data.stage00, stage01: data.stage01 },
      ...
      setData(prev => ({ ...prev, status: 'script_complete' }));
  ...
  const generateQA = async () => {
    ...
    setData(prev => ({ ...prev, status: 'generating_qa', error: undefined }));
    try {
      const fullScriptStr = data.stage07.map(p => p.content).join('\n\n');
      const qaData = await fetchJson('/api/generate/qa', {
         method: 'POST',
         headers: { 'Content-Type': 'application/json' },
         body: JSON.stringify({ fullScript: fullScriptStr, foundation: { stage00: data.stage00, stage01: data.stage01 } }),
      });
      setData(prev => ({ ...prev, status: 'complete', stage08: qaData }));
      setActiveStage('qa');
    } catch (e: any) {
      console.error("QA Error", e);
      setData(prev => ({ ...prev, status: 'error', error: "QA Failed: " + e.message })); 
    }
  };
  ...
  return (
    <div className="flex h-[100dvh] bg-[#080808] text-[#d4d4d4] font-sans overflow-hidden p-4 select-none gap-6">
      ...
      <main className="flex-1 overflow-auto relative border border-[#222] bg-black">
        ...
        {activeStage === 'setup' && <SetupStage data={data} onGenerate={generateSetup} status={data.status} />}
        {activeStage === 'foundation' && <FoundationView data={data} setData={setData} onGenerateFoundation={generateFoundation} onGenerateOutline={generateOutline} status={data.status} />}
        {activeStage === 'outline' && <OutlineView data={data} setData={setData} onGenerateScenes={generateScenes} status={data.status} />}
        {activeStage === 'scenes' && <ScenesView data={data} setData={setData} onGenerateScript={generateScript} status={data.status} />}
        {activeStage === 'script' && (
           <div className="max-w-4xl mx-auto py-12 px-8 space-y-12">
             ...
             {data.status === 'script_complete' && (
               <div className="mt-8 flex justify-end">
                 <button onClick={generateQA} className="bg-[#f43f5e] hover:bg-[#rose-600] text-black font-bold text-xs font-mono uppercase px-6 py-3 transition-colors">
                   Generate QA Review
                 </button>
               </div>
             )}
